...
parser.add_argument(
    '-x', help="To extract the message from the image.", action="store_true")
parser.add_argument('-p', help="To put in a password.",
                    nargs="?", const="no_password_given")
    ...
    if not args.f:
        ...
        if args.f and args.e:
            ...
            if not args.p:
                ...
                print(
                    "[*] If you don't want to supply the password just type apped -p at the end of the query")
                sys.exit(1)
            else:
...
# python3 ./stegno.py -f secret84.png -x -p i  <----  해석
